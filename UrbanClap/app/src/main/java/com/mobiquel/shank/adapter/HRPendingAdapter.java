...
public class HRPendingAdapter extends RecyclerView.Adapter<HRPendingAdapter.MyViewHolder> implements OnMapReadyCallback {
    ...
    public HRPendingAdapter(Context context, JSONArray techArray, String type, ProgressBarCircularIndeterminate progressBar,RecyclerView techList,TextView noRecordsFound,String source) {
        ...
        public MyViewHolder(View view) {
           ...
           /* code.setOnClickListener(new View.OnClickListener() {
             ...
             //   code.setVisibility(View.VISIBLE);
                ...
                verifyCode.setOnClickListener(new View.OnClickListener()
                {
                    ...
                    public void onClick(View view)
                    {
                        ...
                        openPopUp(code.getText().toString(),technicianMobile.getText().toString());

                       /* if(code.getText().toString().equals(""))
                        {
                            Utils.showToast(HRPendingAdapter.this.context,"Please provide code first!");
                        }
                        else
                        {
            ...
            else if(type.equalsIgnoreCase("MEMBER"))
            {
                ...
                {
                    technicianName.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            try {
                                getTechLocation(techArray.getJSONObject(getLayoutPosition()).getString("id"),techArray.getJSONObject(getLayoutPosition()).getString("name"),techArray.getJSONObject(getLayoutPosition()).getString("mobile"));
                                //openMapUp(techArray.getJSONObject(getLayoutPosition()).getString("technicianId"));
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
    ...
    private void verifyCode(final String technicianMobile,final String code) {
        RequestQueue queue = VolleySingleton.getInstance(HRPendingAdapter.this.context).getRequestQueue();
        ...
        StringRequest requestObject = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    {
                        Utils.showToast(HRPendingAdapter.this.context,responseObject.getString("errorMessage"));
            ...
            { if (progressBar != null && progressBar.isShown()) {
                ...
                Utils.showToast(HRPendingAdapter.this.context, AppConstants.MESSAGES.ERROR_FETCHING_DATA_MESSAGE);
            }
        })
        {
            @Override
            protected Map<String, String> getParams() {
                Preferences.getInstance().loadPreferences(HRPendingAdapter.this.context);
                ...
                {
                    params.put("verificationType","added");
        ...
        if (Utils.isNetworkAvailable(HRPendingAdapter.this.context)) {
        ...
        { if (progressBar != null && progressBar.isShown()) {
            ...
            Utils.showToast(HRPendingAdapter.this.context, AppConstants.MESSAGES.ENABLE_INTERNET_SETTING_MESSAGE);
        }
    }


    private void getTechLocation(final String technicianId,final String techName,final String mobile) {
        RequestQueue queue = VolleySingleton.getInstance(HRPendingAdapter.this.context).getRequestQueue();
        String url= AppConstants.SERVER_URL+"getTechniciansLocation";
        progressBar.setVisibility(View.VISIBLE);
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (responseObject.has("errorCode") && responseObject.getString("errorCode").equals("0"))
                    {
                       /* Utils.showToast(HRPendingAdapter.this.context,responseObject.getString("errorMessage"));
                        getHRReferedTechnicianList("PENDING");*/
                       if(responseObject.getJSONObject("responseObject") !=null)
                       {
                           String lat=responseObject.getJSONObject("responseObject").getString("latitude");
                           String lon=responseObject.getJSONObject("responseObject").getString("longitude");
                           openMapUp(lat,lon,techName+"\n"+mobile);
                       }
                       else
                       {
                           Utils.showToast(context,"No location found in DB!");
                       }
    ...
    private void openPopUp(String code, final String mobile) {
        ...
        okButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {
                    Utils.showToast(context,"Please enter code to verify!");
                }
                else
                {
                    verifyCode(mobile,enterCode.getText().toString());
                    dialog.cancel();
                }
    ...
    private void openMapUp(final String lat,final String lon,final String techName) {
        Log.e("HIT1","HIT");
        final Dialog dialog = new Dialog(context);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setCanceledOnTouchOutside(true);
        dialog.setContentView(R.layout.fragment_map_layout);
        dialog.setCancelable(true);
        final MapFragment mapFragment = (MapFragment)((Activity)context).getFragmentManager().findFragmentById(R.id.map);
        //mapFragment.getMapAsync(this);

        final ImageView closeMap = (ImageView) dialog.findViewById(R.id.closeMapLayout);
        final TextView technicianName = (TextView) dialog.findViewById(R.id.technicianName);
        technicianName.setText(techName);
       ...
       mapFragment.getMapAsync(new OnMapReadyCallback() {
           ...
           public void onMapReady(GoogleMap googleMap) {
               ...
               map = googleMap;
               mo.position(new LatLng(Double.parseDouble(lat), Double.parseDouble(lon)));
               Marker marker = map.addMarker(mo);
               marker.setTitle(techName);
        ...
        closeMap.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                dialog.cancel();
                ((Activity)context).getFragmentManager().beginTransaction().remove(mapFragment).commit();
    ...
    public void onMapReady(GoogleMap googleMap) {
       ...
       double lat = loc.getLatitude();
        double lan = loc.getLongitude();
        LatLng myPosition = new LatLng(lat, lan);

        if (loc == null) {
            Utils.showToast(context, "Wait till GPS is set!");
        } else {
            lat = loc.getLatitude();
                ...
                try {
                    ...
                    if (responseObject.has("errorCode") && responseObject.getString("errorCode").equals("0"))
                    {
                        ...
                        else
                        {
                            ...
                            HRPendingAdapter mAdapter=new HRPendingAdapter(context,techArray,"PENDING",progressBar,techList,noRecordsFound,source);
